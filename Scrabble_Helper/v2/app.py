...
def calculer_points(mot, lettres_reglette):
    points = 0
    lettres_restantes = lettres_reglette.lower()
    jokers = lettres_restantes.count('*')

    for lettre in mot:
        if lettre in lettres_restantes:
            lettres_restantes = lettres_restantes.replace(lettre, '', 1)
            points += valeurs_lettres.get(lettre, 0)
        elif jokers > 0:
            jokers -= 1
        else:
            points += valeurs_lettres.get(lettre, 0)
...
def index():
    ...
    longueur_mot = None
    ordre_points = "aucun"
    lettre_debut = ""  # initialiser la variable lettre_debut
    ...
    if request.method == 'POST':
        ...
        lettre_debut = request.form.get('lettre_debut', '')  # récupérer la lettre de début
        ...
        mots_trouves = [(mot, calculer_points(mot, lettres_reglette)) for mot in sorted(mots_possibles)]

        # Filtrer par lettre de départ
        if lettre_debut:  # Si une lettre de début est précisée
            mots_trouves = [mot for mot in mots_trouves if mot[0][0] == lettre_debut.lower()]
